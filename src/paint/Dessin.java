		...
		MouseListener ml = new MouseListener() {
			...
			public void mouseReleased(MouseEvent e) {
				...
				if (boutons.getDessiner()) {
					//Si on clique pour la premi�re fois, on cr�e la figure et on d�selectionne la pr�c�dente
					if (nbClics == 0) {
	...
	public void paintComponent(Graphics g) {
		...
		if (nbFigures > 0) {
			for (int i = 0; i < nbFigures; i++) {
				...
				// Dessin d'un cercle
				// Calcul de la distance � faire ailleurs
				...
				g.drawOval(positions[0].x - diametre / 2, positions[0].y
						- diametre / 2, diametre, diametre);
				//Si la figure est s�lectionn�e, on dessine les points de s�lection
				if (tabFigures[i].getSelection()) {
					g.drawRect(positions[0].x - 3, positions[0].y - 3, 6, 6);
